/**
 * =============================================================================
 * Boltech Group — MÓDULO FIDUCIARIO DE CUMPLIMIENTO DNC & REPUTACIÓN (10/10)
 * =============================================================================
...
const DNC_SECRET = process.env.DNC_SECRET || process.env.FIDUCIARY_SECRET_KEY || 'boltech_fiduciary_dnc_secret_2026';
 ...
 */
export function generateComplianceFooterHtml(email, baseUrl = 'https://boltech-group.vercel.app') {
  ...
  <div style="margin-top: 32px; padding-top: 16px; border-top: 1px solid #e2e8f0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; font-size: 11px; color: #64748b; line-height: 1.5;">
    <p style="margin: 0 0 6px 0;">Este mensaje técnico fue emitido como diagnóstico defensivo por <strong>Boltech Group / Unblock AI</strong> para directores de operaciones y tecnología.</p>
...
export function generateComplianceFooterText(email, baseUrl = 'https://boltech-group.vercel.app') {
  const token = generateUnsubscribeToken(email);
  const cleanBaseUrl = baseUrl.replace(/\/+$/, '');
  const unsubUrl = `${cleanBaseUrl}/api/unsubscribe?token=${token}`;

  return `\n---\nEste mensaje técnico es emitido por Boltech Group como diagnóstico defensivo.\nPara no recibir más análisis, haz clic aquí para darte de baja: ${unsubUrl}\n`;
}

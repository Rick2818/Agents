...
export async function createStripeCheckoutSession({ planId, customerEmail, domain, successUrl, cancelUrl }) {
  ...
  if (!stripeKey) {
    ...
    return {
      sessionId: `concierge_session_${intent.transactionId}`,
      url: `https://wa.me/50375743444?text=Hola%20Boltech%20Group,%20solicito%20activar%20mi%20licencia%20de%20${encodeURIComponent(plan.name)}%20por%20$${plan.amountUSD}%20USD%20-%20Tx:${intent.transactionId}`,

...
// CatÃ¡logo Inmutable de Precios Fiduciarios en USD (Cero manipulaciÃ³n de montos por el cliente)
export const CATALOGO_PRECIOS_USD = Object.freeze({
  'flash_audit_19': {
    id: 'flash_audit_19',
    name: 'Flash Audit License (DiagnÃ³stico en RAM)',
    amount_usd: 19.00,
    tier: 'flash_audit'
  },
  ...
  'inventory_logistics_69': {
    ...
    name: 'Agente de Control de Inventario & EnvÃ­os (LogÃ­stica)',
    amount_usd: 69.00,
    tier: 'inventory_logistics'
  },
  'outbound_sales_79': {
    id: 'outbound_sales_79',
    name: 'Agente de Ventas Outbound & ProspecciÃ³n B2B',
    amount_usd: 79.00,
    tier: 'outbound_sales'
  },
  'financial_audit_89': {
    id: 'financial_audit_89',
    name: 'Agente de Cobranza Fiduciaria & ConciliaciÃ³n Bancaria',
    amount_usd: 89.00,
...
// Ledger de Idempotencia en RAM (Previene ataques de repeticiÃ³n o doble acreditaciÃ³n)
...
/**
 * ValidaciÃ³n criptogrÃ¡fica Timing-Safe en tiempo constante
 */
...
export class StrikeLightningGateway {
  ...
  /**
   * Genera un enlace LNURL / Invoice Lightning para liquidaciÃ³n instantÃ¡nea en USD hacia rick2818@strike.me
   */
  async createLightningPayment(productId, clientEmail = '') {
    const product = CATALOGO_PRECIOS_USD[productId];
    if (!product) {
      throw new Error(`Producto invÃ¡lido. ID '${productId}' no existe en el catÃ¡logo fiduciario.`);
    }
    ...
    // Protocolo LNURL-Pay estÃ¡ndar hacia rick2818@strike.me (LiquidaciÃ³n Directa)
    ...
    return {
      ...
      payment_instructions: `Enviar equivalente en satoshis de $${product.amount_usd.toFixed(2)} USD a la direcciÃ³n Lightning: ${this.lightningAddress}`,
  ...
  verifyWebhookSignature(rawBody, signatureHeader) {
    ...
    const computedSignature = crypto
      ...
      .update(rawBody)
      .digest('hex');
    return timingSafeCompare(computedSignature, signatureHeader);
  }
}
...
export class WompiGateway {
  ...
  async createPaymentLink(productId, clientEmail, redirectUrl) {
    const product = CATALOGO_PRECIOS_USD[productId];
    if (!product) {
      throw new Error(`Producto invÃ¡lido. ID '${productId}' no existe en el catÃ¡logo.`);
    }
    ...
    const response = await fetch(`${this.apiUrl}/EnlacePago`, {
      ...
      body: JSON.stringify({
        ...
        configuracion: {
          esMontoEditable: false, // ðŸ”’ ProtecciÃ³n Bancaria: El cliente NUNCA puede modificar el monto
...
/**
 * Validador universal de idempotencia
 */

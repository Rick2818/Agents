/**
 * =============================================================================
 * Boltech Group â€” MOTOR FIDUCIARIO DE PASARELAS Y CIBERSEGURIDAD BANCARIA
    ...
    if (this.apiKey) {
      const strikeRes = await fetch(`${this.apiBase}/invoices`, {
        ...
        body: JSON.stringify({
          ...
          description: `Boltech Group - ${product.name} (Ref: ${correlationId})`,
  ...
  async createPaymentLink(productId, clientEmail, redirectUrl) {
    ...
    const response = await fetch(`${this.apiUrl}/EnlacePago`, {
      ...
      body: JSON.stringify({
        ...
        nombreProducto: `Boltech Group - ${product.name}`,
        urlRedirect: redirectUrl || 'https://boltech.ai/success',
        configuracion: {

...
export function sendViaSmtps({ host, port = 465, user, pass, from, to, subject, text, html, attachments = [], timeoutMs = 15000 }) {
  return new Promise((resolve, reject) => {
      ...
      for (const line of lines) {
        ...
        // Manejo de errores SMTP (4xx o 5xx)
          ...
          if (code === 535 || line.toLowerCase().includes('badcredentials') || line.toLowerCase().includes('username and password not accepted')) {
            return reject(new Error(`Gmail SMTP 535: Credenciales incorrectas. Para Gmail usa una 'Contraseña de Aplicación' de 16 letras generada en myaccount.google.com/apppasswords. Detalle: ${line}`));
          }
    ...
    socket.on('error', (err) => {
      reject(new Error(`Fallo de conexión SMTP con ${cleanHost}:${cleanPort} — ${err.message}`));
    });

    socket.on('timeout', () => {
      socket.destroy();
      reject(new Error(`Timeout de conexión (${timeoutMs}ms) conectando a ${cleanHost}:${cleanPort}`));
    });
  });
}
...
export async function dispatchUniversalEmail({
  ...
  if (isResendReady) {
    try {
      const fromAddress = envConfig.OFFICIAL_FROM_EMAIL || envConfig.SMTP_FROM || 'Destraba AI <onboarding@resend.dev>';

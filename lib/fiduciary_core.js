...
const VOLATILE_MEMORY_SECRET = process.env.SESSION_SECRET || process.env.FIDUCIARY_SECRET_KEY || 'boltech_fiduciary_master_session_2026';

// --- PILAR 6: CORS WHITELIST ESTRICTA (PROHIBIDO '*' EN PAGOS O SESIONES) ---
const ALLOWED_ORIGINS = [
  'https://boltech.ai',
  'https://unblock.ai',
  'https://www.boltech.ai',
...
export function resolveCorsOrigin(requestOrigin, isLocalDev = false) {
  ...
  if (origin.endsWith('.vercel.app') && (origin.includes('rick2818') || origin.includes('boltech') || origin.includes('agents'))) {
    return requestOrigin;
  }
